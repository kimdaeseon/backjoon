...
#include <cmath>
using namespace std;
/*위상 정렬 + for(int next : 배열) 문법
1. 위상정렬한후 저 반복문을 사용해서 한번에 비교하기 point
2. 초기화 잘하기 ;;
*/
...
int main()
{
	...
	for (int w = 0; w < tc; w++) {
		...
		while (indegree[target]>0) {
			int u = q.front();
			q.pop();
			for (int next : v[u]) {
				ans[next] = max(ans[next], ans[u] + time[u]);
				if (--indegree[next] == 0) q.push(next);
			}
		}
		cout << time[target] + ans[target] << endl;
	}
	return 0;
}
